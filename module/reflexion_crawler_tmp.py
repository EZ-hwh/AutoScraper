...
class AutoCrawler:
        ...
        for reflection_index in range(reflection_times):
            if ground_truth:
                ...
                if self.rule_pattern in ['selector', 'code']:
                    res = self.request_parse(query, html_content, ['thought', 'consistent', 'value', self.rule_pattern])
                else:
                    res = self.request_parse(query, html_content, ['thought', 'consistent', 'value', 'xpath'])
                if self.verbose:
    ...
    def generate_rule(self, instruction, html_content, ground_truth = None, max_token=8000):
        ...
        subtree_list = domlm_parse(soup, 8000)
